...
class Settings(BaseSettings):
    ...
    # =============================================================================
    # DOCX 파서 조직별 규칙 (선택 사항, 미설정 시 해당 규칙 비활성화)
    # =============================================================================
    # 이 정규식과 일치하는 텍스트 뒤에 마크다운 구분선(---)을 추가
    DOCX_SECTION_DIVIDER_PATTERN: Optional[str] = os.getenv(
        "DOCX_SECTION_DIVIDER_PATTERN"
    )
    # 표 셀의 하이퍼링크 중 이 호스트를 포함한 링크만 위키 링크로 인라인 처리
    DOCX_WIKI_LINK_HOST: Optional[str] = os.getenv("DOCX_WIKI_LINK_HOST")
