...
JWT 기반 사용자 인증 및 권한 관리를 위한 Python 라이브러리입니다. 마이크로서비스 아키텍처에서 공통으로 사용되는 **2단계 인가(Authorization) 플로우**를 제공합니다.
...
git submodule add https://github.com/your-org/passport-utils.git app/utils/passport_utils

# 또는 특정 브랜치 지정
git submodule add -b main https://github.com/your-org/passport-utils.git app/utils/passport_utils
...
```bash
# 특정 서비스에 복사
cp -r passport_utils your_service/utils/

# 가져오기
from utils.passport_utils.parser import (
    authorize_request_with_exception,
    require_authorization
)
```
